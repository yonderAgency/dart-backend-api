...
const ProviderLogDb = ProviderLog.getDatastore().manager;

const getRecentBasedIds = async (approvedProviderIds, userId) => {
    var providers = await ProviderLogDb.collection('providerlog')
        .aggregate([
            {
                $match: {
                    created_by: userId,
    ...
    if (providers && providers.length > 0) {
        return _.map(providers, 'provider_id');
    }
    return [];
};
...
module.exports = async function search(req, res) {
    ...
    try {
        ...
        approvedProviderIds = await getRecentBasedIds(
            approvedProviderIds,
            userId
        );
